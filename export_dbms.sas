...
Fails, SAS dataset sashelp.foo does not exist.
...
%if not (%sysfunc(exist(&_temp,DATA)) or %sysfunc(exist(&_temp,VIEW))) %then %do;
   %parmv(_msg=SAS dataset &_temp does not exist)
   %goto quit;
%end;
